...
for ( i in 1:length(lambda) ) {
  ...
  ccc <- rbind(ccc, inar1.get.UL2(ARL0, lambda = lambda[i], beta = 0.5, target="lambda", OUTPUT=FALSE, eps=1e-6, delta=1e-4))
}
...
par(mar=c(4,4,1,4))
plot(lambda, ccc$L, ylim=c(0, max(ccc$L)*1.5), xlab=expression(lambda[0]), ylab="LCL", cex=.1)
par(new=T)
...
par(mar=c(4,4,1,4))
plot(lambda, ccc$U, ylim=c(0, max(ccc$U)*1.5), xlab=expression(lambda[0]), ylab="UCL", cex=.1)
par(new=T)
