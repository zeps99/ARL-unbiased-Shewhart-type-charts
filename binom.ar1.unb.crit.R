...
binom.ar1.unb.crit <- function(n, p, rho, arl0){
  LUg <- ar1.get.UL2(arl0, n, p, rho, target="p", OUTPUT=FALSE, eps=1e-6, delta=1e-4)
  L  <- LUg$L
...
# Example Figure 7, MoraisWittenbergCruz2023
arl0 <- 370.4
n <- 15
p0 <- 0.38
rho0 <- 0.97
binom.ar1.unb.crit(n, p0, rho0, arl0)


####### PARA TABELA TESE #######

# Define the binom.ar1.unb.crit function
binom.ar1.unb.crit <- function(n, p, rho, arl0){
  LUg <- ar1.get.UL2(arl0, n, p, rho, target="p", OUTPUT=FALSE, eps=1e-6, delta=1e-4)
  L  <- LUg$L
  gL <- round(LUg$gL, digits=6)
  U  <- LUg$U
  gU <- round(LUg$gU, digits=6)
  result <- paste("(n, p0, rho0):", n, p, rho, "(LCL,UCL):", L, U, "(gL, gU):", round(gL,6), round(gU,6))
  return(result)
}
...
for (n in n_values) {
  for (p in p0_values) {
    for (rho in rho0_values) {
      ...
      result <- binom.ar1.unb.crit(n, p, rho, arl0)
