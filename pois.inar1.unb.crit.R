# Defining the control limits and randomization probabilities of ARL unbiased Poisson INAR(1)-chart
...
pois.inar1.unb.crit <- function(arl0, lambda0, beta0, target){
...
beta0 <- 0.81
target <- "beta"
pois.inar1.unb.crit(arl0, lambda0, beta0, target)

####### PARA TABELA TESE #######

# Define the pois.inar1.unb.crit function
pois.inar1.unb.crit <- function(arl0, lambda0, beta0, target){
  LUg <- inar1.get.UL2(arl0, lambda0, beta0, target)
  L  <- LUg$L
  gL <- round(LUg$gL, digits=6)
  U  <- LUg$U
  gU <- round(LUg$gU, digits=6)
  result <- paste("(λ0, β0):", lambda0, beta0, "(LCL,UCL):", L, U, "(gL, gU):", round(gL,6), round(gU,6))
  return(result)
}

# Define the values for lambda0
lambda0_values <- c(0.5, 1, 5, 10, 15)

# Define the values for beta0
beta0_values <- c(0.1, 0.25, 0.5)

# Target list
target_list <- c("lambda","beta")
...
for (lambda0 in lambda0_values) {
  for (beta0 in beta0_values) {
    for (target in target_list) {
      ...
      result <- pois.inar1.unb.crit(arl0, lambda0, beta0, target)
...
pois.inar1.unb.crit(arl0, lambda0 = 0.5, beta0 = 0, target = "lambda")
pois.inar1.unb.crit(arl0, lambda0 = 1, beta0 = 0, target = "lambda")
pois.inar1.unb.crit(arl0, lambda0 = 5, beta0 = 0, target = "lambda")
pois.inar1.unb.crit(arl0, lambda0 = 10, beta0 = 0, target = "lambda")
