...
geom.inar1.unb.arl <- function(p0, rho0, arl0, p, rho){
  LUg <- ginar1.get.UL2(ARL0 = arl0, lambda = p0, beta = rho0, target="lambda", OUTPUT=FALSE, eps=1e-6, delta=1e-4)
  L  <- LUg$L
  gL <- round(LUg$gL, digits=6)
  U  <- LUg$U
  gU <- round(LUg$gU, digits=6)
  arl <- round(ginar1.arl2(L, U, gL, gU, lambda = p, beta = rho), digits=4)
  return(arl)
}

#ginar1.get.UL2(ARL0 = 200, lambda = 0.2, beta = 0.5, target="lambda", OUTPUT=FALSE, eps=1e-6, delta=1e-4)

# Define the range and steps for lambda and beta
p_seq <- seq(0.1, 0.5, by=0.01)  # Example range for lambda
rho_seq <- seq(0.1, 0.5, by=0.01)    # Example range for beta
...
for (i in 1:length(p_seq)) {
  ...
  for (j in 1:length(rho_seq)) {
    ...
    result <- ginar1.get.UL2(ARL0, lambda = p, beta = rho, target="lambda", OUTPUT=FALSE, eps=1e-6, delta=1e-4)
...
L_matrix <- read.table("C:/Users/j-gue/OneDrive/Área de Trabalho/2º Ano - Mestrado Matemática Aplicada e Computação/2º Semestre/Dissertação/Codes R/ARL Unbiased/3 AR(1) Shewhart (Done)/3 Geometric (Done)/3D Plot/L_matrix_output.txt", header = TRUE, sep = " ", stringsAsFactors = FALSE)
U_matrix <- read.table("C:/Users/j-gue/OneDrive/Área de Trabalho/2º Ano - Mestrado Matemática Aplicada e Computação/2º Semestre/Dissertação/Codes R/ARL Unbiased/3 AR(1) Shewhart (Done)/3 Geometric (Done)/3D Plot/U_matrix_output.txt", header = TRUE, sep = " ", stringsAsFactors = FALSE)
#gL_matrix <- read.table("C:/Users/j-gue/OneDrive/Área de Trabalho/2º Ano - Mestrado Matemática Aplicada e Computação/2º Semestre/Dissertação/Codes R/ARL Unbiased/3 AR(1) Shewhart (Done)/3 Geometric (Done)/3D Plot/gL_matrix_output.txt", header = TRUE, sep = " ", stringsAsFactors = FALSE)
#gU_matrix <- read.table("C:/Users/j-gue/OneDrive/Área de Trabalho/2º Ano - Mestrado Matemática Aplicada e Computação/2º Semestre/Dissertação/Codes R/ARL Unbiased/3 AR(1) Shewhart (Done)/3 Geometric (Done)/3D Plot/gU_matrix_output.txt", header = TRUE, sep = " ", stringsAsFactors = FALSE)
...
persp(p_seq, rho_seq, L_matrix, theta = 40, phi = 15, expand = 0.5, col = "lightgray",
      xlab = "p0", ylab = "ρ0", zlab = "LCL", zlim = c(-1, 1),
...
persp(p_seq, rho_seq, U_matrix, theta = 30, phi = 15, expand = 0.5, col = "lightgray",
      ...
      cex.lab = 1,  # Adjust axis label size
      mgp = c(2, 0.5, 0))
persp(p_seq, rho_seq, gL_matrix, theta = 30, phi = 20, expand = 0.5, col = "lightgray",
      xlab = "p0", ylab = "ρ0", zlab = "ɣL",
      ticktype = "detailed", # Use detailed ticks to avoid overlay
      cex.axis = 0.5, # Adjust axis label size if needed
      cex.lab = 1,  # Adjust axis label size
      mgp = c(2, 0.5, 0))
persp(p_seq, rho_seq, gU_matrix, theta = 30, phi = 20, expand = 0.5, col = "lightgray",
...
p_seq <- seq(0.1, 0.5, by=0.01)
rho_seq <- seq(0.1, 0.5, by=0.01)
...
for (i in 1:length(rho_seq)) {
  ...
  for (j in 1:length(p_seq)) {
    ...
    arl_result <- geom.inar1.unb.arl(p0, rho0 = rho_seq[i], arl0 = ARL0, p = p_seq[j], rho = rho_seq[i])
...
arl_matrix <- read.table("C:/Users/j-gue/OneDrive/Área de Trabalho/2º Ano - Mestrado Matemática Aplicada e Computação/2º Semestre/Dissertação/Codes R/ARL Unbiased/3 AR(1) Shewhart (Done)/3 Geometric (Done)/3D Plot/arl_matrix_output.txt", header = TRUE, sep = " ", stringsAsFactors = FALSE)
arl_matrix <- as.matrix(arl_matrix)

# Create the 3D plot using persp
persp(rho_seq, p_seq, arl_matrix, theta = 60, phi = 10, expand = 0.4, col = "lightgray",
...
persp_result <- persp(rho_seq, p_seq, arl_matrix, theta = 110, phi = 10, expand = 0.4, col = "lightgray",
                      ylab = "p", xlab = "ρ0", zlab = "ARL",
                      ticktype = "detailed", # Use detailed ticks to avoid overlay
                      cex.axis = 0.5, # Adjust axis label size if needed
                      cex.lab = 1,  # Adjust axis label size
...
for (i in 1:length(p_seq)) {
  ...
  for (j in 1:length(rho_seq)) {
    ...
    ARL0 <- 370.4
    arl_result <- geom.inar1.unb.arl(p0 = p_seq[i], rho0 = rho0, arl0 = ARL0, p = p_seq[j], rho = rho_seq[i])
    arl_matrix[i, j] <- arl_result
  }
}
...
# Create the 3D plot using persp
persp(p_seq, rho_seq, arl_matrix, theta = 70, phi = 10, expand = 0.4, col = "lightgray",
      xlab = "p0", ylab = "ρ", zlab = "ARL",
      ticktype = "detailed", # Use detailed ticks to avoid overlay
      cex.axis = 0.5, # Adjust axis label size if needed
      cex.lab = 1,  # Adjust axis label size
      mgp = c(2, 0.5, 0))
...
# Your existing code to generate the 3D plot
persp_result <- persp(p_seq, rho_seq, arl_matrix, theta = 50, phi = 10, expand = 0.4, col = "lightgray",
                      xlab = "p0", ylab = "ρ", zlab = "ARL",
                      ticktype = "detailed", # Use detailed ticks to avoid overlay
                      cex.axis = 0.5, # Adjust axis label size if needed
                      cex.lab = 1,  # Adjust axis label size
...
# Define the range and steps for d_lambda and d_beta
d_p_seq <- seq(-0.2, 0.2, length.out = 20)  # Example range for d_lambda
d_rho_seq <- seq(-0.2, 0.2, length.out = 20)    # Example range for d_beta

# Create a matrix to store L values
arl_matrix <- matrix(0, nrow = length(d_p_seq), ncol = length(d_rho_seq))

# Fill the L_matrix using nested loops
for (i in 1:length(d_p_seq)) {
  for (j in 1:length(d_rho_seq)) {
    ARL0 <- 370.4
    d_p <- d_p_seq[i]
    d_rho <- d_rho_seq[j]
    p0 <- 0.3
    ...
    arl_result <- geom.inar1.unb.arl(p = p0, rho = rho0, arl0 = ARL0, d_p, d_rho)
    arl_matrix[i, j] <- arl_result
...
# Create the 3D plot using persp
persp(d_p_seq, d_rho_seq, arl_matrix, theta = 40, phi = 15, expand = 0.4, col = "lightgray",
      xlab = "δp", ylab = "δρ", zlab = "ARL")
