# Defining the ARL unbiased c-chart value
...
arl2 <- Vectorize(function(cL, cU, gL, gU, lambda) {
  aL <- F(cL-1, lambda) + gL*dpois(cL, lambda)
  aU <- 1 - F(cU, lambda) + gU*dpois(cU, lambda)
  L  <- 1 / ( aL + aU )
  L
}, "lambda")

#' Compute ARLs of unbiased c-charts (in and out-control)
#'
#' @description
#' Computation of the Average Run Length (ARL) at a given mean mu0, ARL0 and delta.
#'
#'
...
#'
#' @return it returns the ARL value of a unbiased c-chart in-control (delta = 0) or out-control (delta ≠ 0)
#' @export
#'
#' @author something
#'
...
#' arl0 <- 370.4
#' delta <- 0
#' pois.unb.arl(mu0,arl0,delta)
pois.unb.arl <- function(mu0,arl0,delta){
  results <- search.GandC(mu0+delta, (arl0)^(-1))
  cL4  <- results$cL
  cU4  <- results$cU
  gL4  <- results$gL
  gU4  <- results$gU
  ARL4 <- arl2(cL4, cU4, gL4, gU4, mu0)
  return(ARL4)
}