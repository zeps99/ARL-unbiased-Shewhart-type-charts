######################################################################################
### ScottoWeissSilvaPereira2013 - weekly rainy days Bremen (Germany) (2000 - 2004) ### 
######################################################################################
...
qcc <- function(data, type = c("xbar", "R", "S", "xbar.one", "p", "np", "c", "u", "g"), sizes, center, std.dev, limits, data.name, labels, newdata, newsizes, newdata.name, newlabels, nsigmas = 3, confidence.level, rules = shewhart.rules, plot = TRUE, ...)
{
  ...
  object$gaminhas <- gammas(arl0 = 500, n = 208, p0 = 0.01784393, rho0 = 0.1635034) ############################									   
                     ...
                     restore.par = TRUE, ...) 
{
  ...
  { 
    ...
    ##obtaining the randomization probabilities with arl0 = 500
    gaminhas <- gammas(arl0 = 500, n = 208, p0 = 0.01784393, rho0 = 0.1635034) ############################									   
...
{ 
  ...
  ## ADDED
  pbar <- 0.01784393
  limites <- limites(arl0 = 500, n = 208, p0 = pbar, rho0 = 0.1635034)
  lcl<-limites[,1]
  ucl<-limites[,2]
...
#######################################################################
###### ARL nearly-unbiased binomial AR(1) profile with REAL DATA ######
#######################################################################

library(ggplot2)
library(readxl)

data1 <- read_excel("rainy.xlsx")
data_mat <- as.matrix(data1[,2])
head(data_mat)
data_vec <- c(data_mat)


# YW estimators 
n <- as.numeric(length(data_vec))
p0_yw <- mean(data_vec)/n
p0_yw # 0.01784393
...
ggplot(df_long, aes(x = Value, y = Count, fill = Dataset)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("Data Counts" = "#2D2D2D", "binomial Fit" = "#BFBFBF")) +  # Darker gray and lighter gray
  theme(
...
### +- follows a binomial distribution (preciso de checkar)

### calculate rho -> ver weisskim2013 (Yule-Walker estimate and cls) # ALGO ESTÀ MAL AQUI 

##################################
##### YULE-WALKER ESTIMATORS #####
##################################

diff <- data_vec - n*p0_yw
...
rho0_yw > -p0_yw/(1-p0_yw)

center <- p0_yw*n
center

# SO OUR YW INFORMATION IS
# p0 = 0.01784393
# n = 208
# rho = 0.1635034
# arl0 = 500
...
ar1.get.UL2(ARL0 = 500, n = n, p = p0_yw, rho = rho0_yw, target="p", OUTPUT=FALSE, eps=1e-6, delta=1e-4)

p1 <- p0_yw + 0.01
dados = c(rbinom(n=30, size=208, p1))
qcc(data_vec[199:208], type="np", sizes=208, newdata=dados[1:30],newsizes=208, ylab="Nonconforming items", xlab="Group", title="ARL-unbiased binomial AR(1)-chart")
...
calculate_rho_cls <- function(X) {
  ...
  sum_Xt_Xt1 <- sum(X[2:T] * X[1:(T-1)])
  sum_Xt1 <- sum(X[2:T])
  sum_Xt <- sum(X[2:T])
  sum_Xt1_sq <- sum(X[2:T] ^ 2)
  
  numerator <- (T - 1) * sum_Xt_Xt1 - sum_Xt1 * sum_Xt
  denominator <- (T - 1) * sum_Xt1_sq - sum_Xt1^2
...
rho0_cls <- calculate_rho_cls(data_vec)
rho0_cls # 0.1725654

calculate_p_cls <- function(X) {
  T <- length(X)
  n <- T  # Here n is the same as T, since the whole time series is used
  ...
  numerator <- sum_Xt - rho_hat_CLS * sum_Xt1
  denominator <- n * (T - 1) * (1 - rho_hat_CLS)
  
  p_hat_CLS <- numerator / denominator
  
  return(p_hat_CLS)
}

p0_cls <- calculate_p_cls(data_vec)
p0_cls # 0.01782755

ar1.get.UL2(ARL0 = 500, n = n, p = p0_cls, rho = rho0_cls, target="p", OUTPUT=FALSE, eps=1e-6, delta=1e-4)
