####################################################################
############# Barreto-SouzaNdrecaSilvaSilva2023 #################### 
### monthly counts Hansen's disease Paraíba (Brazil) (2001-2021) ### 
####################################################################
...
qcc <- function(data, type = c("xbar", "R", "S", "xbar.one", "p", "np", "c", "u", "g"), sizes, center, std.dev, limits, data.name, labels, newdata, newsizes, newdata.name, newlabels, nsigmas = 3, confidence.level, rules = shewhart.rules, plot = TRUE, ...)
{
  ...
  object$gaminhas <- gammas(arl0 = 200, p0 = 1/center, rho0 = 0) ############################									   
                     ...
                     restore.par = TRUE, ...) 
{
  ...
  { 
    ...
    gaminhas <- gammas(arl0 = 200, p0 = 1/center, rho0 = 0) 
...
limits.g <- function(center, std.dev, sizes, conf)
{
  ...
  if (length(unique(sizes)) == 1)
    sizes <- sizes[1]
  pbar <- 1/center
  limites <- limites(arl0 = 200, p0 = pbar, rho0 = 0)
  lcl<-limites[,1]
  ucl<-limites[,2]
  limits <- matrix(c(lcl, ucl), ncol = 2)
  rownames(limits) <- rep("", length = nrow(limits))
...
violating.runs <- function(object, run.length = qcc.options("run.length"), limits=object$limits)
{
  ...
  gaminhas <- gammas(arl0 = 200, p0 = 1/center, rho0 = 0) #alterado
  gL<-gaminhas[,1]
  gU <- gaminhas[,2]
...
data <- read_excel("data_disease.xlsx")
data_mat <- as.matrix(data)
head(data_mat)
data_vec <- c(data_mat)

p0 <- 1/(1+mean(data_vec)) #mt pequeno, vai demorar horrores
p0
...
ginar1.get.UL2(ARL0 = arl0, lambda = p0, beta = rho0, target="lambda", OUTPUT=FALSE, eps=1e-6, delta=1e-4)


dados  <- c(rgeom(n=10, prob = p0), rgeom(n=30, prob = p1))
dados
