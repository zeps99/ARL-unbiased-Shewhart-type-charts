...
ar1.get.UL2(ARL0, n, p = 0.428, rho, target="p", OUTPUT=FALSE, eps=1e-6, delta=1e-4) 

ARL0 <- 370.4
n <- 15
rho <- 0.5
p <- seq(0.01, 0.5, by=0.0006)
ccc <- NULL

for ( i in 1:length(p) ) {
  cat(paste(i, "\t", p[i], "\n"))
  ccc <- rbind(ccc, ar1.get.UL2(ARL0, n, p[i], rho, target="p", OUTPUT=FALSE, eps=1e-6, delta=1e-4))
}
...
par(mar=c(4,4,1,4))
plot(p, ccc$L, ylim=c(0, max(ccc$L)*1.5), xlab=expression(p[0]), ylab="LCL", cex=.1)
par(new=T)
