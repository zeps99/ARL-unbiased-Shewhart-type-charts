...
# Paulino, Morais & Knoth (2019) - Algorithm
...
qcc <- function(data, type = c("xbar", "R", "S", "xbar.one", "p", "np", "c", "u", "g"), sizes, center, std.dev, limits, data.name, labels, newdata, newsizes, newdata.name, newlabels, nsigmas = 3, confidence.level, rules = shewhart.rules, plot = TRUE, ...)
{
  ...
  object$gaminhas <- gammas(arl0 = 500, lambda0 = center, beta0 = 0.81, target="lambda") ############################									   
                     ...
                     restore.par = TRUE, ...) 
{
  ...
  { 
    ...
    gaminhas <- gammas(arl0 = 500, lambda0 = center, beta0 = 0.81, target="lambda") 
...
limits.c <- function(center, std.dev, sizes, conf)
{
  ...
  if(length(unique(sizes))==1)
    sizes <- sizes[1]
  limites <- limites(arl0 = 500, lambda0 = center, beta0 = 0.81, target="lambda")
  lcl <- limites[1,1]
  ucl <- limites[1,2]
...
violating.runs <- function(object, run.length = qcc.options("run.length"), limits=object$limits)
{
  ...
  gaminhas <- gammas(arl0 = 500, lambda0, beta0 = 0.81, target="lambda")
  gL<-gaminhas[,1]
  gU <- gaminhas[,2]
...
##################################################################
###### ARL-unbiased Poisson INAR(1)-chart w/ SIMULATED DATA ######
##################################################################

#target_lambda = 0.4636
#target_beta = 0.81
#LCL= 0
#UCL=9
#gL=0.0170696
...
# se quiser fazer com outros valores tenho de substituir em cima o arl0 e o beta!
lambda0 <- 0.4636
beta0 <- 0.81
mu0 <- lambda0/(1-beta0)
arl0 <- 500
inar1.get.UL2(ARL0 = arl0, lambda = lambda0, beta = beta0, target="lambda", OUTPUT=FALSE, eps=1e-6, delta=1e-4)

shiftinmu <- 4
shiftinlambda <- shiftinmu*(1-beta0)
shiftinlambda

Z <- rpois(1, mu0)

dados  <- c(rbinom(30, Z, beta0)) + c(rpois(n=10, lambda = lambda0), rpois(n=20, lambda = lambda0 + shiftinlambda))
length(dados)
obj <- qcc(dados[1:10],type="c", newdata = dados[11:30], center=0.4636, ylab="Number of nonconformities", xlab="Sample number", title="ARL-unbiased Poisson INAR(1)-chart")
