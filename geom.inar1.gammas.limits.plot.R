...
ARL0 <- 200
rho <- 0.5
p <- seq(0.1, 0.5, by=0.001)
ccc <- NULL

#ginar1.get.UL2(ARL0, lambda = 0.1, beta = rho, target="lambda", OUTPUT=FALSE, eps=1e-6, delta=1e-4)

for ( i in 1:length(p) ) {
  cat(paste(i, "\t", p[i], "\n"))
  ccc <- rbind(ccc, ginar1.get.UL2(ARL0, lambda = p[i], beta = rho, target="lambda", OUTPUT=FALSE, eps=1e-6, delta=1e-4))
}

dL  <- c(1, which( diff(ccc$L) > 0 ) + 1) #da os indices onde ? verdadeiro, ou seja, onde h? altera??o do L 
gL  <- l2 <- NULL
for ( i in 1:(length(dL)-1) ) {
  gL <- c(gL, ccc$gL[dL[i]:(dL[i+1]-1)], NA) #da os gL e mete NA entre as altera??es de L
  l2 <- c(l2, p[dL[i]:(dL[i+1]-1)], NA) #da os p e mete NA entre as altera??es de L
}
...
l2 <- c(l2, p[max(dL):nrow(ccc)]) #junta os ultimos p depois da ultima altera??o do L, uma vez que n?o s?o adicionados anteriormente 

dU  <- c(1, which( abs(diff(ccc$U)) > 0 ) + 1)
gU  <- l3 <- NULL
for ( i in 1:(length(dU)-1) ) {
  gU <- c(gU, ccc$gU[dU[i]:(dU[i+1]-1)], NA)
  l3 <- c(l3, p[dU[i]:(dU[i+1]-1)], NA)
}
gU <- c(gU, ccc$gU[max(dU):nrow(ccc)])
l3 <- c(l3, p[max(dU):nrow(ccc)])

par(mar=c(4,4,1,4))
plot(p, ccc$L, ylim=c(0, 1), xlab=expression(p[0]), ylab="LCL", cex=.3)
par(new=T)
plot(l2, gL, type="l", col="black", axes=F, bty="n", xlab="", ylab="")
abline(h=0:0.05, lty=4, col="grey")
abline(v=0.1, lty=4, col="grey")
abline(v=0.5, lty=4, col="grey")
axis(4, at=pretty(c(0,0.05)))
mtext(expression(gamma[L]), 4, 3)
graphics.off()

par(mar=c(4,4,1,4))
plot(p, ccc$U, ylim=c(0, max(ccc$U)*1.5), xlab=expression(p[0]), ylab="UCL", cex=.1)
par(new=T)
plot(l3, gU, type="l", col="black", axes=F, bty="n", xlab="", ylab="", pch=16, cex=0.2)
